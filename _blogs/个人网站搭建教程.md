...
title设置了页面的标题，permalink设置了页面的绝对路径。标题与绝对路径要和栏目的设置一致。\{\% include base_path \%\}同样是供github pages生成网页使用，不需要修改。在阅读给github pages生成网页的指令时，\{\%和\%\}均不需要斜杠\\，因为网页中无法使用花括号与百分号的组合。
